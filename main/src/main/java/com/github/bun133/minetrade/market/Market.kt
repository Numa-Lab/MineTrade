...
private const val DEFAULT_INTERVAL_TICK = 20 * 30
class Market(private val conf: MineTradeConfig,val plugin: JavaPlugin) {
    ...
    fun getEntryFor(block: Block): MarketEntry? {
        ...
        return map.getOrPut(e) { MarketEntry(e.block, e.item, e.basePrice,conf.isRealMode,DEFAULT_INTERVAL_TICK,plugin) }
    }
